...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 2C5BE9CC11
	for <lists+ceph-devel@lfdr.de>; Mon, 26 Aug 2019 11:01:42 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1730426AbfHZJBk (ORCPT <rfc822;lists+ceph-devel@lfdr.de>);
        Mon, 26 Aug 2019 05:01:40 -0400
Received: from m97138.mail.qiye.163.com ([220.181.97.138]:52739 "EHLO
        m97138.mail.qiye.163.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1726354AbfHZJBk (ORCPT
        <rfc822;ceph-devel@vger.kernel.org>); Mon, 26 Aug 2019 05:01:40 -0400
Received: from yds-pc.domain (unknown [218.94.118.90])
        by smtp9 (Coremail) with SMTP id u+CowAAnZGTEn2Nd3xqAAQ--.17S2;
        Mon, 26 Aug 2019 17:00:53 +0800 (CST)
...
References: <1564393377-28949-1-git-send-email-dongsheng.yang@easystack.cn>
 ...
 <5D6349E8.8080704@easystack.cn>
Cc:     Jason Dillaman <jdillama@redhat.com>,
        Ceph Development <ceph-devel@vger.kernel.org>
From:   Dongsheng Yang <dongsheng.yang@easystack.cn>
Message-ID: <5D639FC4.4050601@easystack.cn>
Date:   Mon, 26 Aug 2019 17:00:52 +0800
User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:38.0) Gecko/20100101
 Thunderbird/38.5.0
MIME-Version: 1.0
In-Reply-To: <5D6349E8.8080704@easystack.cn>
Content-Type: text/plain; charset=utf-8; format=flowed
Content-Transfer-Encoding: 7bit
X-CM-TRANSID: u+CowAAnZGTEn2Nd3xqAAQ--.17S2
X-Coremail-Antispam: 1Uf129KBjvAXoWfAF4fCF1fWr1xJw13WF1rZwb_yoW5Cw1UAo
        W5Kr1xuFs5GFy7XF47Gr97tw4rX348WayrArW5KF4ak3ZIya4UZ3y7Jr4rAF45Aw48Crs7
        t3WxJwnavF4qqw15n29KB7ZKAUJUUUUU529EdanIXcx71UUUUU7v73VFW2AGmfu7bjvjm3
        AaLaJ3UbIYCTnIWIevJa73UjIFyTuYvjTRGksgUUUUU
X-Originating-IP: [218.94.118.90]
X-CM-SenderInfo: 5grqw2pkhqwhp1dqwq5hdv52pwdfyhdfq/1tbiRgUdelbdGyao0wAAsp
...
On 08/26/2019 10:54 AM, Dongsheng Yang wrote:
>
>
...
>>>
>>> diff --git a/include/linux/ceph/journaler.h 
>>> b/include/linux/ceph/journaler.h
>>> index e3b82af..f04fb3f 100644
>>> --- a/include/linux/ceph/journaler.h
>>> +++ b/include/linux/ceph/journaler.h
>>> @@ -173,7 +173,7 @@ static inline uint64_t 
>>> ceph_journaler_get_max_append_size(struct ceph_journaler
>>>   struct ceph_journaler_ctx *ceph_journaler_ctx_alloc(void);
>>>   void ceph_journaler_ctx_put(struct ceph_journaler_ctx 
>>> *journaler_ctx);
...
>>>   void ceph_journaler_client_committed(struct ceph_journaler 
>>> *journaler,
>>>                                       uint64_t commit_tid);
>> These declarations should be added in this patch, not earlier.
>
> They should be. I put them in generic journaling commit just for my 
> lazy :p
...
>>> @@ -29,6 +29,42 @@ static char *object_oid_prefix(int pool_id, const 
>>> char *journal_id)
>>>          return prefix;
>>>   }
>>>
...
>>> + *
>>> + * Append starts in JOURNALER_APPEND_START and ends in 
>>> JOURNALER_APPEND_FINISH
>>> + */
...
>>> +
>>>   /*
>>>    * journaler_append_ctx is an internal structure to represent an 
>>> append op.
>>>    */
...
>>>   };
>>>
>>> @@ -151,6 +188,12 @@ struct ceph_journaler 
>>> *ceph_journaler_create(struct ceph_osd_client *osdc,
...
>>> +       INIT_DELAYED_WORK(&journaler->commit_work, 
>>> journaler_client_commit);
>>> +       INIT_WORK(&journaler->notify_update_work, 
>>> journaler_notify_update);
...
>>> @@ -1287,3 +1330,794 @@ int ceph_journaler_start_replay(struct 
>>> ceph_journaler *journaler)
>>>          return ret;
>>>   }
...
>>> +}
>>> +
>>> +static uint64_t get_object(struct ceph_journaler *journaler, 
>>> uint64_t splay_offset)
>>> +{
>>> +       return splay_offset + (journaler->splay_width * 
...
>>> +               prev_future_finished = 
>>> (journaler->prev_future->consistent &&
...
>>> +}
>>> +
>>> +static void journaler_entry_encode_prefix(struct 
>>> ceph_journaler_entry *entry,
>>> +                                         void **p, void *end)
>>> +{
...
>>> +static void journaler_append_finish(struct work_struct *work)
>>> +{
>>> +       struct ceph_journaler *journaler = container_of(work, struct 
>>> ceph_journaler,
>>> + finish_work);
...
>>> +}
>>> +
>>> +static void journaler_handle_append(struct journaler_append_ctx 
>>> *ctx, int ret);
...
>>> +}
>>> +
>>> +static void journaler_append_callback(struct ceph_osd_request 
>>> *osd_req)
>>> +{
...
>>> +       if (ret)
>>> +               goto out_free_page;
>>> +       osd_req_op_cls_request_data_pages(req, 0, &ctx->req_page, 8, 
>>> 0, false, false);
>>> +
>>> +       // append_data
>>> +       osd_req_op_extent_init(req, 1, CEPH_OSD_OP_APPEND, 0,
>>> +               ctx->journaler_ctx.prefix_len + 
>>> ctx->journaler_ctx.bio_len + ctx->journaler_ctx.suffix_len, 0, 0);
>>> +
...
>>> &ctx->journaler_ctx.bio_iter, ctx->journaler_ctx.bio_len);
>>> +
...
>>> +       }
>>> +
>>> +       ret = append(journaler, &object_oid, &journaler->data_oloc, 
>>> ctx);
>>> +out:
...
>>> +                       list_del(&ctx->node);
>>> +                       ctx->object_num = get_object(journaler, 
>>> obj_recorder->splay_offset);
...
>>> +       // As we don't do advance in flushing, so queue another 
>>> overflow_work
...
>>> +}
>>> +
>>> +static void ceph_journaler_object_append(struct ceph_journaler 
>>> *journaler,
...
>>> +       struct object_recorder *obj_recorder;
>>> +
>>> +       // PEAMBLE(8) + version(1) + entry_tid(8) + tag_tid(8) + 
>>> string_len(4) = 29
...
>>> +       end = buf + 4;
>>> +       crc = crc32c(crc, page_address(journaler_ctx->prefix_page) + 
>>> journaler_ctx->prefix_offset,
>>> +                    journaler_ctx->prefix_len);
>>> +       if (journaler_ctx->bio_len)
>>> +               crc = crc_bio(crc, bio_iter->bio, 
>>> journaler_ctx->bio_len);
...
>> bio?
>
> ->bio_iter in journaler_ctx is advanced for this chunk when split 
> event into chunks in rbd level.

Sorry, this is buggy. I just advance the bio_iter, but use bio_iter->bio 
here. that means
you are right, every chunk would checksumming from start of bio. That's 
not correct.

And also, the reason I did not found it in my testing, because there is 
no muliti-events
appending in rbd_mirror.sh. I will fix it in next version.

Thanx
>>> +       ceph_encode_32(&buf, crc);
>>> +       obj_recorder = 
>>> &journaler->obj_recorders[append_ctx->splay_offset];
>>> +
>>> +       spin_lock(&obj_recorder->lock);
...
>>> +               spin_unlock(&obj_recorder->lock);
>>> +               ret = send_append_request(journaler, 
>>> ctx->object_num, ctx);
>>> +               if (ret) {
>>> +                       pr_err("failed to send append request: %d", 
>>> ret);
...
>>> +               spin_unlock(&obj_recorder->lock);
>>> +
>>> +               ret = add_commit_entry(journaler, 
>>> ctx->future.commit_tid, ctx->object_num,
>>> +                                      ctx->future.tag_tid, 
...
>>> +       struct ceph_journaler_ctx *journaler_ctx;
>>> +
>>> +       append_ctx = kmem_cache_zalloc(journaler_append_ctx_cache, 
>>> GFP_NOIO);
>>> +       if (!append_ctx)
>>> +               return NULL;
>>> +
...
>>> +       struct ceph_journaler_ctx *journaler_ctx;
>>> +
>>> +       append_ctx = container_of(kref, struct journaler_append_ctx, 
>>> kref);
>>> +       journaler_ctx = &append_ctx->journaler_ctx;
>>> +
>>> +       __free_page(journaler_ctx->prefix_page);
...
>>> +}
>>> +
>>> +static void journaler_append_ctx_put(struct journaler_append_ctx 
>>> *append_ctx)
>>> +{
>>> +       if (append_ctx) {
>>> +               kref_put(&append_ctx->kref, 
>>> journaler_append_ctx_release);
...
>>> +       obj_recorder = 
>>> &journaler->obj_recorders[journaler->splay_width];
>>> +       append_ctx->object_num = get_object(journaler, 
>>> append_ctx->splay_offset);
>>> +
>>> +       // allocate a commit_tid for this event, when the data is 
>>> committed
>>> +       // to data objects, ceph_journaler_client_committed() will 
>>> accept
>>> +       // the commit_tid to understand how to update journal commit 
>>> position.
>>> +       journaler_ctx->commit_tid = __allocate_commit_tid(journaler);
>>> +       entry_init(&append_ctx->entry, tag_tid, entry_tid, 
>>> journaler_ctx);
>>> +
...
>>> +               }
>>> +
>>> +               if (splay_offset == position->object_num % 
>>> splay_width) {
>>> +                       found = true;
>>> +                       break;
>>> +               }
