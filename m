...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 81D4614D536
	for <lists+ceph-devel@lfdr.de>; Thu, 30 Jan 2020 03:23:15 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726694AbgA3CXM (ORCPT <rfc822;lists+ceph-devel@lfdr.de>);
        Wed, 29 Jan 2020 21:23:12 -0500
Received: from us-smtp-delivery-1.mimecast.com ([207.211.31.120]:47899 "EHLO
        us-smtp-1.mimecast.com" rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org
        with ESMTP id S1726485AbgA3CXM (ORCPT
        <rfc822;ceph-devel@vger.kernel.org>); Wed, 29 Jan 2020 21:23:12 -0500
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
        s=mimecast20190719; t=1580350990;
        h=from:from:reply-to:subject:subject:date:date:message-id:message-id:
         to:to:cc:cc:mime-version:mime-version:content-type:content-type:
         content-transfer-encoding:content-transfer-encoding:
         in-reply-to:in-reply-to:references:references;
        bh=XI4dnlbpMBMAaHeX7vl+qYTGZ/TcZyXr6UKdCljJC6k=;
        b=dXe4t+ONmGgGwlG8kqcFe7yfX3Ly5BTqLCXCimdCz6zV0WFEuMGsCrLxPRE8LOXZTgwNij
        E9sXorILLUqUbcNL9jsETLK5PE+uDefSJlYpEcoHJaS0yj6L1XBQnJHvqHWGJDYxBDayzQ
        bkWf1vI7YFHaRSyyybvXEyo9+NiOLHg=
Received: from mimecast-mx01.redhat.com (mimecast-mx01.redhat.com
 [209.132.183.4]) (Using TLS) by relay.mimecast.com with ESMTP id
 us-mta-415-b-EggoPOOqK1G0wWhidPyw-1; Wed, 29 Jan 2020 21:23:00 -0500
X-MC-Unique: b-EggoPOOqK1G0wWhidPyw-1
Received: from smtp.corp.redhat.com (int-mx04.intmail.prod.int.phx2.redhat.com [10.5.11.14])
        (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
        (No client certificate requested)
        by mimecast-mx01.redhat.com (Postfix) with ESMTPS id 8F9A28010C7;
        Thu, 30 Jan 2020 02:22:59 +0000 (UTC)
Received: from [10.72.12.34] (ovpn-12-34.pek2.redhat.com [10.72.12.34])
        by smtp.corp.redhat.com (Postfix) with ESMTPS id 4DF595DA75;
        Thu, 30 Jan 2020 02:22:54 +0000 (UTC)
...
Message-ID: <c60f2ad9-1b33-04d5-8b65-e4205880b345@redhat.com>
Date:   Thu, 30 Jan 2020 10:22:49 +0800
User-Agent: Mozilla/5.0 (Windows NT 10.0; WOW64; rv:68.0) Gecko/20100101
 Thunderbird/68.4.1
MIME-Version: 1.0
In-Reply-To: <a456a29671efa7a94a955bc8f1655bb042dbf13d.camel@kernel.org>
...
>> --- a/fs/ceph/acl.c
>> +++ b/fs/ceph/acl.c
>> @@ -22,6 +22,8 @@ static inline void ceph_set_cached_acl(struct inode =
*inode,
...
>> --- a/fs/ceph/addr.c
>> +++ b/fs/ceph/addr.c
>> @@ -1706,6 +1706,8 @@ int ceph_uninline_data(struct file *filp, struct=
 page *locked_page)
>>   			err =3D -ENOMEM;
>>   			goto out;
>>   		}
...
>> + */
> This needs some comments. Specifically, what should this be counting an=
d
> how?

Will add it.

The __ceph_caps_metric() will traverse the inode's i_caps try to get the=20
'issued' excepting the 'invoking' caps until it get enough caps in=20
...
> caps for pAsFsLs. We have two MDS's and they've each granted us caps fo=
r
> the inode, say:
>
> MDS 0: pAs
...
Only the i_cap0 is 'hit'.=C2=A0 i_cap1 will not be counted any 'hit' or '=
mis'.
...
The 'hit' means to get all the caps in 'mask we have checked the=20
i_cap[0~N] and if they have a subset in 'mask', and the 'mis' means we=20
have checked the i_cap[0~N] but they do not.

For the i_cap[N+1 ~ M], if we won't touch them because we have already=20
gotten enough caps needed in 'mask', they won't count any 'hit' or 'mis'

All in all, the current logic is that the 'hit' means 'touched' and it=20
have some of what we needed, and the 'mis' means 'touched' and missed=20
...
> cap->issued fields together, and then check that vs. the mask value, an=
d
...
>>    * out, and may be invalidated in bulk if the client session times o=
ut
>> @@ -2746,6 +2815,7 @@ static void check_max_size(struct inode *inode, =
loff_t endoff)
...
>> @@ -2758,6 +2828,7 @@ int ceph_try_get_caps(struct inode *inode, int n=
eed, int want,
...
>>   }
>> @@ -2784,6 +2855,8 @@ int ceph_get_caps(struct file *filp, int need, i=
nt want,
...
>> @@ -2871,6 +2944,7 @@ int ceph_get_caps(struct file *filp, int need, i=
nt want,
...
>> @@ -137,6 +138,25 @@ static int metric_show(struct seq_file *s, void *=
p)
...
>> --- a/fs/ceph/dir.c
>> +++ b/fs/ceph/dir.c
>> @@ -313,7 +313,7 @@ static int ceph_readdir(struct file *file, struct =
dir_context *ctx)
...
>>  =20
>> @@ -346,13 +346,16 @@ static int ceph_readdir(struct file *file, struc=
t dir_context *ctx)
...
>>   	}
>>  =20
>> @@ -757,6 +760,8 @@ static struct dentry *ceph_lookup(struct inode *di=
...
>> --- a/fs/ceph/file.c
>> +++ b/fs/ceph/file.c
>> @@ -384,6 +384,8 @@ int ceph_open(struct inode *inode, struct file *fi=
le)
>>   	 * asynchronously.
>>   	 */
>>   	spin_lock(&ci->i_ceph_lock);
...
>> @@ -1340,6 +1342,7 @@ static ssize_t ceph_read_iter(struct kiocb *iocb=
, struct iov_iter *to)
>>   				return -ENOMEM;
>>   		}
>>  =20
...
>> @@ -558,6 +558,8 @@ void ceph_put_mds_session(struct ceph_mds_session =
*s)
...
>>   	}
>>   }
>> @@ -598,6 +600,7 @@ static struct ceph_mds_session *register_session(s=
truct ceph_mds_client *mdsc,
>>   						 int mds)
>>   {
>>   	struct ceph_mds_session *s;
>> +	int err;
...
>> @@ -612,8 +615,10 @@ static struct ceph_mds_session *register_session(=
...
>> @@ -653,6 +658,13 @@ static struct ceph_mds_session *register_session(=
struct ceph_mds_client *mdsc,
>>  =20
...
/
>> @@ -662,6 +674,8 @@ static struct ceph_mds_session *register_session(s=
truct ceph_mds_client *mdsc,
>>  =20
>>   	return s;
>>  =20
...
>> --- a/fs/ceph/quota.c
>> +++ b/fs/ceph/quota.c
>> @@ -147,9 +147,14 @@ static struct inode *lookup_quotarealm_inode(stru=
...
>> --- a/fs/ceph/super.h
>> +++ b/fs/ceph/super.h
>> @@ -641,6 +641,14 @@ static inline bool __ceph_is_any_real_caps(struct=
 ceph_inode_info *ci)
>>   	return !RB_EMPTY_ROOT(&ci->i_caps);
>>   }
>>  =20
>> +extern void __ceph_caps_metric(struct ceph_inode_info *ci, int mask);
>> +static inline void ceph_caps_metric(struct ceph_inode_info *ci, int m=
ask)
>> +{
>> +	spin_lock(&ci->i_ceph_lock);
...
>> +}
>> +
>>   extern int __ceph_caps_issued(struct ceph_inode_info *ci, int *imple=
mented);
>>   extern int __ceph_caps_issued_mask(struct ceph_inode_info *ci, int m=
ask, int t);
>>   extern int __ceph_caps_issued_other(struct ceph_inode_info *ci,
>> @@ -927,6 +935,9 @@ extern int __ceph_do_getattr(struct inode *inode, =
struct page *locked_page,
>>   			     int mask, bool force);
>>   static inline int ceph_do_getattr(struct inode *inode, int mask, boo=
l force)
>>   {
>> +	struct ceph_inode_info *ci =3D ceph_inode(inode);
>> +
...
>> --- a/fs/ceph/xattr.c
>> +++ b/fs/ceph/xattr.c
>> @@ -829,6 +829,7 @@ ssize_t __ceph_getxattr(struct inode *inode, const=
...
>> @@ -856,7 +857,9 @@ ssize_t __ceph_getxattr(struct inode *inode, const=
 char *name, void *value,
>>  =20
...
>> @@ -871,6 +874,9 @@ ssize_t __ceph_getxattr(struct inode *inode, const=
 char *name, void *value,
>>   		if (err)
>>   			return err;
>>   		spin_lock(&ci->i_ceph_lock);
...
>> @@ -907,19 +913,24 @@ ssize_t ceph_listxattr(struct dentry *dentry, ch=
ar *names, size_t size)
>>   	struct ceph_inode_info *ci =3D ceph_inode(inode);
